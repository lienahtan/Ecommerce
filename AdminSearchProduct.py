...
class AdminSearchProduct:
    ...
    def __init__(self, root):
        ...
        self.root = root
        self.root.title("Admin Search Product System")
        self.root.geometry("1350x700+0+0")
        self.root.resizable(False, False)

        title = Label(self.root, text="Product Search System", font=("calibri", 40, "bold"), bg="dodger blue", fg="white")
        ...
        db_frame = Frame(self.root, bd=4, relief=RIDGE, bg="light green")
        db_frame.place(x=20, y=100, width=450, height=560)

        db_title = Label(db_frame, text="Admin Search product", bg="light green", fg="grey", font=("calibri", 25, "bold"))
        db_title.grid(row=0, columnspan=1, pady=20)

        # ======Manage request=========

        label_search_by = Label(db_frame, text="Search by", bg="light green", fg="grey", font=("calibri", 12, "bold"))
        ...
        label_search_value = Label(db_frame, text="Search Value", bg="light green", fg="grey", font=("calibri", 12, "bold"))
        ...
        label_filter = Label(db_frame, text="Filter (optional, default as everything)", bg="light green", fg="grey", font=("calibri", 16, "bold"))
        label_filter.grid(row=3, column=0, pady=10, padx=10, sticky="w")

        label_price = Label(db_frame, text="Price", bg="light green", fg="grey", font=("calibri", 12, "bold"))
        ...
        label_color = Label(db_frame, text="Color", bg="light green", fg="grey", font=("calibri", 12, "bold"))
        ...
        combo_color = ttk.Combobox(db_frame, textvariable=self.color, font=("calibri", 12, "bold"),
                                   state="readonly")
        combo_color['values'] = ("", "grey", "Black", "Blue", "Yellow", "Green")
        combo_color.place(x=150, y=280)

        label_factory = Label(db_frame, text="Factory", bg="light green", fg="grey", font=("calibri", 12, "bold"))
        ...
        label_productionYear = Label(db_frame, text="Production Year", bg="light green", fg="grey", font=("calibri", 12, "bold"))
        ...
        label_powerSupply = Label(db_frame, text="Power Supply", bg="light green", fg="grey",
                                     font=("calibri", 12, "bold"))
        ...
        reqbutton = Frame(db_frame, bd=4, relief=RIDGE, bg="light green")
        reqbutton.place(x=15, y=500, width=420)

        button_search = Button(reqbutton, text="Search", width=20, command=self.admin_search_product)
        button_search.grid(row=0, column=1, padx=20, pady=10)

        # ======Detail Frame=======

        self.detail_frame = Frame(self.root, bd=4, relief=RIDGE, bg="light green")
        self.detail_frame.place(x=500, y=100, width=800, height=560)

        lable_productResult = Label(self.detail_frame, text="Product Search Result", bg="light green", fg="grey", font=("calibri", 20, "bold"))
        lable_productResult.grid(row=0, column=0, pady=20, padx=7, sticky="w")

        # ====== Table Frame ========

        table_frame = Frame(self.detail_frame, bd=4, relief=RIDGE, bg="light green")
        ...
        self.product_table = ttk.Treeview(table_frame,
                                          columns=("Category", "Product Model",
                                                   ...
                                                   "Inventory","Num of item sold","Cost"),
        ...
        if attribute == "":
            ...
            try:
                ...
                if len(cursorList) != 0:
                    num_emptyProduct = 0
                    for x in cursorList:
                        filter1List = list(itemcol.find({'$and':[{'Category': x['Category']}, {'Model': x['Model']}]}))
                        ...
                        collection_filter7 = collection_filter6
                        inventory = len(list(collection_filter6.find({'PurchaseStatus': 'Unsold'})))
                        numOfItemSold = len(list(collection_filter7.find({'PurchaseStatus': 'Sold'})))
                        if inventory != 0 or numOfItemSold !=0:
                            self.product_table.insert('', END, values=(x['Category'], x['Model'],
                                                                x['Price'], x['Warranty'], inventory, numOfItemSold, x['Cost']))
                        else:
                        ...
                        collection_filter5.delete_many({})
                        collection_filter6.delete_many({})
                        collection_filter7.delete_many({})
                    ...
                    if num_emptyProduct == len(cursorList):
                        ...
                        lable_searchResult = Label(self.detail_frame, text=str(searchResult), bg="light green",
                                                   fg="grey",
