...
class AdminDB:
    ...
    def __init__(self, root):
        ...
        txt_id = Entry(db_frame, bd = 5, textvariable=self.id_var, relief = GROOVE, bg="white", fg="black", font=("calibri",12,"bold"))
        ...
        txt_model = Entry(db_frame, bd = 5, textvariable=self.model_var, relief = GROOVE, bg="white", fg="black", font=("calibri",12,"bold"))
        ...
        txt_category = Entry(db_frame, bd = 5, textvariable=self.category_var, relief = GROOVE, bg="white", fg="black", font=("calibri",12,"bold"))
        ...
        txt_price = Entry(db_frame, bd = 5, textvariable=self.price_var, relief = GROOVE, bg="white", fg="black", font=("calibri",12,"bold"))
        ...
        txt_cost = Entry(db_frame, bd = 5, textvariable=self.cost_var, relief = GROOVE, bg="white", fg="black", font=("calibri",12,"bold"))
        ...
        txt_warranty = Entry(db_frame, bd = 5, textvariable=self.warranty_var, relief = GROOVE, bg="white", fg="black", font=("calibri",12,"bold"))
        ...
        txt_inventory = Entry(db_frame, bd = 5, textvariable=self.inventory_var, relief = GROOVE, bg="white", fg="black", font=("calibri",12,"bold"))
        ...
        addBtn = Button(prodbutton, text="Add", width=10,command=self.add_products).grid(row=0, column=0, padx=10, pady=10)
        updateBtn = Button(prodbutton, text="Update", width=10, command=self.update_data).grid(row=0, column=1, padx=10, pady=10)
        deleteBtn = Button(prodbutton, text="Delete", width=10, command=self.delete_data).grid(row=0, column=2, padx=10, pady=10)
        clearBtn = Button(prodbutton, text="Clear", width=10, command=self.clear).grid(row=0, column=3, padx=10, pady=10)
        ...
        combo_search=ttk.Combobox(detail_frame, textvariable=self.search_by, font=("calibri",12,"bold"),state="readonly")
        combo_search['values']=("productID", "productModel", "category")
        combo_search.grid(row=0, column=1,padx=20,pady=10)

        txt_Search = Entry(detail_frame, textvariable=self.search_txt, font=("calibri",12,"bold"), bd=5, relief=GROOVE)
        ...
        self.product_table=ttk.Treeview(table_frame,
                                        columns=("productID", "productModel",
                                                 "category", "price",
                                                 "cost", "warrantyDuration"),
        ...
        self.product_table.heading("productModel", text="Model")
...
##            ))
    ...
    def search_data(self):
        con=mysql.connector.connect(host="localhost", user="root", password="s63127734", database="oshes")
        cur=con.cursor()
        cur.execute("select * from Product where "+str(self.search_by.get())+" LIKE '%"+str(self.search_txt.get())+"%'")
        rows=cur.fetchall()
        if len(rows) != 0:
            self.product_table.delete(*self.product_table.get_children())
            for rows in rows:
