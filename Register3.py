...
class Register:
    ...
    def __init__(self, root):
       ...
       # self.root.config(bg="white")
        ...
        #Username
        custID = Label(frame1, text = "Username", font=("calibri",15,"bold"),bg="white", fg="gray")
        custID.place(x=50, y=50)

        self.custID = Entry(frame1, font=("calibri",15), bg="lightgray")
        self.custID.place(x=50, y=80, width=250)
        ...
        self.name.place(x=50, y=150, width=250)
        ...
        self.email = Entry(frame1, font=("calibri",15), bg="lightgray")
        self.email.place(x=50, y=220, width=250)
        ...
        self.address = Entry(frame1, font=("calibri",15), bg="lightgray")
        self.address.place(x=50, y=290, width=350)
    ...
    def register_data(self):
        if (self.custID.get() == "" or self.name.get() == "" or self.num.get() == "" or self.email.get() == ""
        or self.address.get() == "" or self.gender_cmb.get() == "Select"
        or self.address.get() == "" or self.pw.get() == "" or self.cpw.get() == "") :
            ...
            try:
                ...
                cur=con.cursor()
                cur.execute("insert into Customer (customerID, name, gender, email, phoneNumber, address, password) values(%s,%s,%s,%s,%s,%s,%s)",
                            (self.custID.get(),
                             self.name.get(),
                             self.gender_cmb.get(),
                             self.email.get(),
